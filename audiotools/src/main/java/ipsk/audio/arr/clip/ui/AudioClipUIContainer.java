...
public class AudioClipUIContainer extends JPanel implements Scrollable,
...
//		rb = ResourceBundle.getBundle(resBundleName);
	...
	public AudioClipUIContainer(AudioClip audioSample)
	...
	public void setAudioClip(AudioClip audioSample) {
		...
		if (audioSample != null) {
			...
			// }
			audioClipChanged(new AudioSourceChangedEvent(this, audioSample
					.getAudioSource()));
		...
		// super.setAudioSample(audioSample);
		preferredWidth = (int) (lengthInSeconds * xZoom);
		...
		// plugin.setAudioSample(audioSample);
		// }
		// }
		if (xScale != null) {
		...
		audioClipChanged(new AudioSourceChangedEvent(this, as));
		xZoomFitToPanel();
		repaint();
...
//					resized();
	...
	private void doXZoomFitToPanel(){
		...
		if(length > 0 && lengthInSeconds >0) {
			xZoom=preferredWidth/lengthInSeconds;
		}
	}
					...
					if(c==pc){
					    if (plugin.isPreferredFixedHeight()) {
					        ...
					        if(pc.isVisible()) {
					            ...
					            pc.setBounds(0, availableHeight - fbHeight, width,
					                    fbHeight);
